...
public class ServerPlayer
{
	...
	public float GetRotation()
	{
		...
		if (null != m_vehicle)
		{
			result = m_vehicle.transform.rotation.eulerAngles.y;
		}
		else if (IsSpawned())
		{
	...
	public void ConsumeItem(int a_itemType)
	{
		if (Items.IsMedicine(a_itemType))
		{
			...
			{
			case 140:
				SetCondition(eCondition.bleeding, a_state: false);
				break;
			case 141:
				SetCondition(eCondition.infection, a_state: false);
				break;
			case 142:
				SetCondition(eCondition.pain, a_state: false);
				break;
			case 143:
				m_healEndTime = Time.time + Items.GetItemDef(a_itemType).healing / m_healHealthGainPerSec;
				SetCondition(eCondition.pain, a_state: false);
				SetCondition(eCondition.bleeding, a_state: false);
				break;
			}
		...
		{
			ChangeEnergyBy(Items.GetItemDef(a_itemType).healing);
		}
	}
	...
	public void Progress(float a_deltaTime)
	{
		...
		{
			bool flag2 = Time.time > m_killXp[i].deletetime;
			if ((null != m_killXp[i].npc && m_killXp[i].npc.GetHealth() == 0f) || (m_killXp[i].player != null && m_killXp[i].player.IsDead()) || flag2)
			{
				if (!flag2)
				{
					AddXp((int)m_killXp[i].xp);
				}
				m_killXp.RemoveAt(i);
				break;
			}
	...
	public float ChangeHealthBy(float a_delta)
	{
		...
		if (IsDead() && m_respawnTime < 0f)
		{
			...
			if (IsSpawned())
			{
				m_char.AssignInput(0f, 0f, a_use: false, a_space: false);
	...
	private float HandleDamage(float a_healthDif)
	{
		...
		if (num < -2f)
		{
			...
			if (Mathf.Abs(num) > Random.Range(0f, 140f))
			{
				...
				{
				case 0:
					SetCondition(eCondition.bleeding, a_state: true);
					break;
				case 1:
					SetCondition(eCondition.infection, a_state: true);
					break;
				case 2:
					SetCondition(eCondition.pain, a_state: true);
					break;
				}
			}
		}
	...
	private void HandleRotation()
	{
		...
		if (IsAttacking())
		{
			if (null != m_victim)
			{
				num = Quaternion.LookRotation(m_victim.position - m_char.transform.position).eulerAngles.y;
			}
	...
	private void DropLootAsContainer()
	{
		if (m_inventory != null)
		{
			...
			{
				m_server.CreateTempContainerItem(m_inventory.m_items[i].type, m_inventory.m_items[i].amount, GetPosition());
			}
	...
	private bool FireWeapon(ItemDef a_weaponItem)
	{
		...
		if (a_weaponItem.ammoItemType > 0)
		{
			...
			if (m_inventory.m_items.Count > 0)
			{
				for (int i = 0; i < m_inventory.m_items.Count; i++)
				{
					if (m_inventory.m_items[i].type == a_weaponItem.ammoItemType)
					{
	...
	private void CalculateXpAndKarma(Transform a_victim, float a_weaponDamage)
	{
		...
		for (int i = 0; i < m_killXp.Count; i++)
		{
			if (m_killXp[i].player == null && serverNpc == m_killXp[i].npc)
			{

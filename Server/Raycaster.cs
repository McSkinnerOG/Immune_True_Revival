...
public static class Raycaster
{
	public static float Attack(Transform a_aggressor, ItemDef a_weapon, Vector3 a_targetPos, ref Transform a_target)
	{
		...
		Vector3 vector = ((!(null != a_target)) ? a_targetPos : a_target.position);
		vector.y = 0f;
		bool flag = 0 == a_weapon.ammoItemType;
		Vector3 vector2 = Vector3.up * ((!flag) ? 1.5f : 0.8f);
		...
		if (vector4.sqrMagnitude > 3.5f || null == a_target)
		{
			int layerMask = -5;
			RaycastHit hitInfo;
			if (flag)
			{
				Collider[] array = Physics.OverlapSphere(a_targetPos + vector2, 0.4f, layerMask);
				if (array != null && 0 < array.Length)
				{
					transform = array[0].transform;
				}
			}
			else if (Physics.Raycast(position, vector4.normalized, out hitInfo, a_weapon.range, layerMask))
			{
				transform = hitInfo.transform;
				float num2 = vector4.sqrMagnitude / (a_weapon.range * a_weapon.range) * 0.5f + 0.5f;
	...
	public static bool CheckSphere(Vector3 a_pos, float a_radius)
	{
		...
		{
			string text = "CheckSphere colliders: ";

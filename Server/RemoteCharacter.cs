...
public class RemoteCharacter : MonoBehaviour
{
		...
		case EResult.k_EResultOK:
		{
			...
			if (punOutItemsArraySize != 0)
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					...
					{
						m_itemPopupGui.ShowGui(a_show: true, array[i].m_iDefinition.m_SteamItemDef);
						break;
					}
				}
			}
	...
	private void LateUpdate()
	{
		...
		{
			Vector3 vector = m_labelOffset;
			if (null != m_animControl && m_animControl.m_isSitting)
			{
				vector = Vector3.up * 1000f;
			}
			m_label.transform.position = base.transform.position + vector;
	...
	{
		GameObject prefab = GetPrefab(a_isDead: true);
		if (null != prefab)
		{
			Vector3 vector = Vector3.up * ((!(null == m_animControl) && !m_animControl.m_isSitting) ? 1f : 0.1f);
			Object.Instantiate(prefab, base.transform.position + vector, base.transform.rotation);
		}
	}

...
public class SQLThreadManager : MonoBehaviour
{
	...
	public void SavePlayer(ServerPlayer a_player, bool a_justUpdateParty = false)
	{
		...
		lock (m_threadLock)
		{
			...
			list.Add(new DatabasePlayer(a_player.m_accountId, saveStr(a_player.m_name), a_player.m_pid, a_player.GetPosition().x, a_player.GetPosition().z, (int)(a_player.GetHealth() + 0.5f), (int)(a_player.GetEnergy() + 0.5f), (int)a_player.GetKarma(), a_player.GetXp(), a_player.GetConditions(), a_player.m_gold, a_player.m_partyId, a_player.m_partyRank));
		}
	}
		...
		foreach (int num in a_cids)
		{
			...
			using (IDataReader dataReader = m_sqlCommand.ExecuteReader())
			{
				while (dataReader.Read())
				{
					lock (m_threadLock)
					{
						m_itemAnswers.Add(new DatabaseItem(dataReader.GetInt32(0), dataReader.GetFloat(1), dataReader.GetFloat(2), dataReader.GetInt32(3), a_hidden: false, num, dataReader.GetInt32(4)));
					}
				}
				dataReader.Close();
			}
	...
	private void SQLChangeItems(DatabaseItem[] a_changedItems)
	{
		...
		for (int i = 0; i < a_changedItems.Length; i++)
		{
			...
			{
				if (databaseItem.flag != eDbAction.insert)
				{
					continue;
				}

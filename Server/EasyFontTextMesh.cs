...
public class EasyFontTextMesh : MonoBehaviour
{
	...
	{
		CacheTextVars();
		RefreshMesh(_updateTexureInfo: true);
	}
	...
	private void RefreshMesh(bool _updateTexureInfo)
	{
		...
		if (textMesh != null)
		{
			textMesh.Clear(keepVertexLayout: true);
			SetAnchor();
	...
	private void CreateCharacter(char _character, int _arrayPosition, Vector3 _offset, Color _colorTop, Color _colorBottom)
	{
		...
		if (info.flipped)
		{
			ref Vector3 reference = ref vertices[4 * _arrayPosition];
			reference = new Vector3(vector.x + num2, num3 + vector.y, 0f) + _offset + vector2;
			ref Vector3 reference2 = ref vertices[4 * _arrayPosition + 1];
			reference2 = new Vector3(vector.x, num3 + vector.y, 0f) + _offset + vector2;
			ref Vector3 reference3 = ref vertices[4 * _arrayPosition + 2];
			reference3 = new Vector3(vector.x, vector.y, 0f) + _offset + vector2;
			ref Vector3 reference4 = ref vertices[4 * _arrayPosition + 3];
			reference4 = new Vector3(vector.x + num2, vector.y, 0f) + _offset + vector2;
		}
		else
		{
			ref Vector3 reference5 = ref vertices[4 * _arrayPosition];
			reference5 = new Vector3(vector.x + num2, num3 + vector.y, 0f) + _offset + vector2;
			ref Vector3 reference6 = ref vertices[4 * _arrayPosition + 1];
			reference6 = new Vector3(vector.x, num3 + vector.y, 0f) + _offset + vector2;
			ref Vector3 reference7 = ref vertices[4 * _arrayPosition + 2];
			reference7 = new Vector3(vector.x, vector.y, 0f) + _offset + vector2;
			ref Vector3 reference8 = ref vertices[4 * _arrayPosition + 3];
			reference8 = new Vector3(vector.x + num2, vector.y, 0f) + _offset + vector2;
		}
		List<float> list3;
		...
		if (info.flipped)
		{
			ref Vector2 reference9 = ref uv[4 * _arrayPosition];
			reference9 = new Vector2(info.uv.x, info.uv.y + info.uv.height);
			ref Vector2 reference10 = ref uv[4 * _arrayPosition + 1];
			reference10 = new Vector2(info.uv.x, info.uv.y);
			ref Vector2 reference11 = ref uv[4 * _arrayPosition + 2];
			reference11 = new Vector2(info.uv.x + info.uv.width, info.uv.y);
			ref Vector2 reference12 = ref uv[4 * _arrayPosition + 3];
			reference12 = new Vector2(info.uv.x + info.uv.width, info.uv.y + info.uv.height);
		}
		else
		{
			ref Vector2 reference13 = ref uv[4 * _arrayPosition];
			reference13 = new Vector2(info.uv.x + info.uv.width, info.uv.y);
			ref Vector2 reference14 = ref uv[4 * _arrayPosition + 1];
			reference14 = new Vector2(info.uv.x, info.uv.y);
			ref Vector2 reference15 = ref uv[4 * _arrayPosition + 2];
			reference15 = new Vector2(info.uv.x, info.uv.y + info.uv.height);
			ref Vector2 reference16 = ref uv[4 * _arrayPosition + 3];
			reference16 = new Vector2(info.uv.x + info.uv.width, info.uv.y + info.uv.height);
		}
		if (_privateProperties.customFillMaterial != null)
		{
			Vector2 vector3 = new Vector2(_offset.x, _offset.y);
			Vector2 vector4 = new Vector2(vector2.x, vector2.y);
			ref Vector2 reference17 = ref uv2[4 * _arrayPosition];
			reference17 = new Vector2(vector.x + num2, num3 + vector.y) + vector3 + vector4;
			ref Vector2 reference18 = ref uv2[4 * _arrayPosition + 1];
			reference18 = new Vector2(vector.x, num3 + vector.y) + vector3 + vector4;
			ref Vector2 reference19 = ref uv2[4 * _arrayPosition + 2];
			reference19 = new Vector2(vector.x, vector.y) + vector3 + vector4;
			ref Vector2 reference20 = ref uv2[4 * _arrayPosition + 3];
			reference20 = new Vector2(vector.x + num2, vector.y) + vector3 + vector4;
		}

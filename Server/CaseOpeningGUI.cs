...
public class CaseOpeningGUI : MonoBehaviour
{
	...
	private void Start()
	{
		...
		m_gui.SetActive(value: false);
	}

	public void Showtime(int a_itemDefToWin, int a_generatorDef)
	{
		m_gui.SetActive(value: true);
		m_btnClose.SetActive(value: false);
		...
		if (m_gui.activeSelf)
		{
			...
			if (m_curSpeed > 0.01f)
			{
				for (int i = 0; i < m_displayItems.Length; i++)
				{
					...
					if (m_displayItems[i].transform.localPosition.x > 0.085f)
					{
					...
					if (localPosition.x < 0.015f && 0.015f < m_displayItems[i].transform.localPosition.x && null != base.audio)
					{
						base.audio.Play();
					}
				}
			...
			else if (!m_btnClose.activeSelf)
			{
				...
				if (null != item)
				{
					...
					if (null != m_client)
					{
						m_client.SendChatMsg(":#~" + item["market_name"], a_local: false);
					}
					else
					{
				...
				m_btnClose.SetActive(value: true);
			}
		}

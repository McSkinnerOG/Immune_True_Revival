...
public class ClientInput : MonoBehaviour
{
	...
	private void HandlePopup()
	{
		if (m_buySellPopupSessionId == m_popupGui.GetSessionId())
		{
			...
			{
				m_popupGui.ShowGui(a_show: false, string.Empty);
			}
		}
	...
	private RemoteCharacter FindTarget(RemoteCharacter a_curTarget)
	{
		...
		if (null != m_client)
		{
			...
			if (null != player)
			{
				float num = Mathf.Max(Items.GetItemDef(m_client.GetHandItem()).range, 5f);
				float num2 = num * 0.25f;
				float num3 = num * 0.75f;
				Vector3 vector = player.transform.position + player.transform.forward * num2;
				RemoteCharacter remoteCharacter = m_client.GetNearestNpc(vector);
				if (null != remoteCharacter && (remoteCharacter.transform.position - vector).magnitude > num3)
				{
					remoteCharacter = null;
				}
				RemoteCharacter remoteCharacter2 = null;
				if (null != remoteCharacter2 && (remoteCharacter2.transform.position - vector).magnitude > num3)
				{
				...
				if (null != handItem)
				{
					if (Items.HasCondition(handItem.m_type) && handItem.m_amountOrCond < 100)
					{
						...
						if (m_client.GetGoldCount() >= num)
						{
							...
							m_repairPopupSessionId = m_popupGui.ShowGui(a_show: true, a_caption);
						}
						else
						{
							string a_caption2 = LNG.Get("ITEMSHOP_TOO_LESS_GOLD") + "\n" + num + " " + LNG.Get("CURRENCY");
							m_popupGui.ShowGui(a_show: true, a_caption2);
						}
					}
					else
					{
						m_popupGui.ShowGui(a_show: true, LNG.Get("REPAIR_NPC_NO_NEED"));
					}
				}
				else
				{
					m_popupGui.ShowGui(a_show: true, LNG.Get("REPAIR_NPC_HOWTO"));
				}
				flag = true;
				break;
			}
	...
	{
		m_missionPopupSessionId = m_popupGui.ShowGui(a_show: true, GetMissionText(a_mission));
	}
	...
	private void GetMouseInput(float a_deltaTime)
	{
		...
		{
			if (Physics.Raycast(ray, out var hitInfo, 100f, 665600))
			{
		...
		else if (Input.GetMouseButtonDown(0))
		{
			...
			bool flag4 = false;
			if (Physics.Raycast(ray, out var hitInfo2, 100f, 6995488))
			{
					...
					if (m_inventory.IsShopActive())
					{
						...
						if (null != component4 && component4.m_type != 254 && m_inventory.IsVisible())
						{
							...
							if (flag5 || m_client.GetGoldCount() >= num2)
							{
								...
								m_buySellPopupSessionId = m_popupGui.ShowGui(a_show: true, a_caption);
							}
							else
							{
				...
				case 21:
					if (flag && Input.GetAxis("Vertical") == 0f && Input.GetAxis("Horizontal") == 0f && null != m_client && !m_client.IsInVehicle())
					{
						...
						m_walkIndicator.position = new Vector3(hitInfo2.point.x, 0.1f, hitInfo2.point.z);
		...
		else if (!flag2)
		{
			...
			if (Physics.Raycast(ray, out var hitInfo3, 100f, 7007776))
			{
				...
				else
				{
					...
					else if (hitInfo3.transform.gameObject.layer == 19)
					{
						...
						if (null != m_buildingHealthIndicator && null != component5)
						{
							...
							m_buildingHealthIndicator.renderer.material.mainTextureOffset = new Vector2(x, 0f);
							m_buildingHealthIndicator.position = hitInfo3.transform.position + vector;
						}
					}
					else if (hitInfo3.transform.gameObject.layer == 15)
					{
						...
						Vector3 vector2 = Vector3.up * ((!flag6) ? 3f : 6.5f);
						m_tooltipText.text = LNG.Get("TOOLTIP_RESOURCE");
						m_tooltip.position = hitInfo3.transform.position + vector2;
					}
					else if (hitInfo3.transform.gameObject.layer == 21)
					{
						Vector3 vector3 = Vector3.up * 2f;
						m_tooltipText.text = LNG.Get("TOOLTIP_ITEMSTORAGE");
						m_tooltip.position = hitInfo3.transform.position + vector3;
					}
					else if (hitInfo3.transform.gameObject.layer == 9)
					{
						...
						if (null == component6)
						{
							Vector3 vector4 = Vector3.up * 3f;
							m_tooltipText.text = LNG.Get("TOOLTIP_INTERACT");
							m_tooltip.position = hitInfo3.transform.position + vector4;
						}
					}
					else if (hitInfo3.transform.gameObject.layer == 22)
					{
						...
						if (null != component7)
						{
							Vector3 vector5 = Vector3.up * 3f;
							m_tooltipText.text = LNG.Get("MISSION_TYPE_" + component7.m_type.ToString("d"));
							m_tooltip.position = hitInfo3.transform.position + vector5;
						}
					}
		...
		if (null != m_client && !m_client.IsInVehicle())
		{
			...
			if (NavMesh.CalculatePath(m_client.GetPos(), a_target, -1, m_path))
			{
				if (m_path.corners.Length > 0 && (m_path.corners[m_path.corners.Length - 1] - a_target).sqrMagnitude > 1f)
				{
					...
					ref Vector3 reference = ref m_pathCorners[0];
					reference = m_client.GetPos();
	...
	private void CalculateRotTowardsMouse(Vector3 a_mousePos)
	{
		...
		{
			Vector3 vector = Camera.main.WorldToScreenPoint(m_client.GetPos() + Vector3.up);
			m_rotTowardsMousePos = Vector3.Angle((a_mousePos - vector).normalized, Vector3.up);
			if (a_mousePos.x < vector.x)
			{

...
public class TrackPoint : MonoBehaviour
{
	...
	public void UpdateTrack()
	{
		...
		{
			Vector3 vector2 = (Vector3)m_prevPoint.m_subPoints[m_prevPoint.m_subPoints.Count - 1];
			vector = (base.transform.position - vector2).normalized;
		}
		if ((bool)m_nextPoint)
		{
	...
	private void CreateSubPoints(Vector3 startLookDir, Vector3 dirToNextPoint)
	{
		...
		for (int i = 1; i < num + 1; i++)
		{
			...
			float num3 = Quaternion.LookRotation(m_nextPoint.transform.position - position).eulerAngles.y - y;
			if (num3 > 180f)
			{
				num3 -= 360f;
			}

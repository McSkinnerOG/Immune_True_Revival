...
public class SimpleVehicle : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		float num5 = m_power * fixedDeltaTime * (num3 * 0.5f + 0.5f) * (num * 0.5f + 0.5f);
		if (axis_v != 0f && magnitude < m_maxSpeedMs)
		{
			base.rigidbody.AddForce(base.transform.forward * axis_v * num5 * (1f - Mathf.Abs(num3) * 0.5f));
		}
		...
		if (magnitude > 1f)
		{
			float num6 = num * (1f - num3);
			float num7 = num2 * num6 * fixedDeltaTime * m_axis_h;
			if (num7 != 0f)
			{
				base.rigidbody.AddTorque(Vector3.up * num7 * m_rotPower);
				m_rotationTarget = -1f;
			}
			else
			{

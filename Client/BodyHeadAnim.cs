...
public class BodyHeadAnim : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		GameObject gameObject = SetupPart(m_bodyPrefab, base.transform, a_optional: false);
		if (null != gameObject)
		{
			...
			{
				GameObject gameObject2 = SetupPart(m_rucksackPrefab, m_boneBody.bone, a_optional: false);
			...
			{
				m_boneHead.lookPart = SetupPart(m_headPrefab, m_boneHead.bone, a_optional: false);
	...
	private void HandleHologram()
	{
		...
		if (m_isSitting)
		{
			m_holoBuilding.SetActive(value: false);
			return;
		}
		if (!m_holoBuilding.activeSelf)
		{
			m_holoBuilding.SetActive(value: true);
		}
		...
		if (a_optional)
		{
			...
			if (a_bone == m_boneHand)
			{
				...
				else
				{
					...
					m_holoBuilding = SetupPart(gameObject3, base.transform, a_optional: false);
					if (null != m_holoBuilding)
					{
	...
	{
		ChangePart(ref m_boneHead, a_optional: true, a_itemIndex);
	}

	public void ChangeHandItem(int a_itemIndex)
	{
		ChangePart(ref m_boneHand, a_optional: true, a_itemIndex);
	}

	public void ChangeBodyItem(int a_itemIndex)
	{
		ChangePart(ref m_boneBody, a_optional: true, a_itemIndex);
	}
}

...
public class ControlledChar : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (m_testMode)
		{
			...
			AssignInput(axis2, axis, a_use: false, a_space: false);
		}
		Move(Time.deltaTime);
	}
	...
	private void Move(float a_dt)
	{
		...
		float num = ((!m_isWalking) ? m_curSpeed : (m_curSpeed * 0.6f));
		Vector3 vector = Vector3.forward * m_axis_v + Vector3.right * m_axis_h;
		if (Vector3.zero != vector)
		{
			if (Time.time > m_ignoreMoveRotTime)
			{
				float num2 = ((!m_isWalking) ? m_rotationSpeed2 : (m_rotationSpeed2 * 0.5f));
				m_rot = Quaternion.Lerp(Quaternion.Euler(0f, m_rot, 0f), Quaternion.LookRotation(vector), a_dt * num2).eulerAngles.y;
			}
			Vector3 vector2 = vector.normalized * num * a_dt;

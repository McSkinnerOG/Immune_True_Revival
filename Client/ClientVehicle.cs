...
public class ClientVehicle : MonoBehaviour
{
	...
	private void Start()
	{
		...
		m_lastRot = base.transform.rotation.eulerAngles.y;
	...
	private void Update()
	{
		...
		float num2 = base.transform.rotation.eulerAngles.y - m_lastRot;
		float num3 = ((!(0f > Vector3.Dot(vector.normalized, base.transform.forward))) ? 1f : (-1f));
	...
	private void SetHealth(float a_health)
	{
		...
		if (m_carPartsStartRot == null)
		{
			...
			{
				ref Quaternion reference = ref m_carPartsStartRot[i];
				reference = m_carParts[i].localRotation;
			}
		}

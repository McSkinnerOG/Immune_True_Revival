...
public class TutorialMessages : MonoBehaviour
{
	...
	private void Update()
	{
		...
		for (int i = 0; i < 11; i++)
		{
			if (Time.time > m_nextMsgShowTime[i] && DisplayMessage((eMsg)i))
			{
				m_nextMsgShowTime[i] = Time.time + m_displayIntervall;
				break;
			}
		}
	}
	...
	private bool DisplayMessage(eMsg a_msg)
	{
		...
		case eMsg.eBuildBuilding2:
			flag = !m_inventoryGui.IsVisible() && Items.GetItemDef(m_client.GetHandItem()).buildingIndex > 0;
			break;
		case eMsg.eShovel:
		...
		case eMsg.eDriveCar:
		{
			RemoteCharacter nearestCharacter2 = m_client.GetNearestCharacter(pos, a_cars: true);

...
public class PlayersOnlineGui : MonoBehaviour
{
	...
	private void Start()
	{
		...
		m_playerEntityTemplate.SetActive(value: false);
	}
	...
	private int UpdateList()
	{
		if (null != m_playerEntityTemplate)
		{
			...
			for (int j = 0; j < 36; j++)
			{
				...
				{
					componentInChildren2.name = "btnpe_" + m_charData[num].aid;
				}
			}
		}
	...
	public bool IsMuted(string a_name)
	{
		for (int i = 0; i < m_mutedSteamIds.Count; i++)
		{
			...
			{
				if (m_mutedSteamIds[i] == m_charData[j].aid && a_name == m_charData[j].name)
				{
					return true;
				}
			}

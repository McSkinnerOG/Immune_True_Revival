...
public class ComChatGUI : MonoBehaviour
{
	...
	private void Start()
	{
		...
		m_unreadMsgIndicator.SetActive(value: false);
		m_unreadMsgIndicator2.SetActive(value: false);
	}

	private void OnGUI()
	{
		m_isActive = m_chatText.gameObject.activeInHierarchy;
		if (!m_qmunicator.IsActive(a_ignoreAnimation: false) || !m_isActive)
		{
			return;
		}
		...
		if (Event.current.keyCode == KeyCode.Return || Event.current.keyCode == KeyCode.KeypadEnter)
		{
			...
			if (m_chatString.Length > 0)
			{
				if (!IsSpam(m_chatString))
				{
					if (null != m_client)
					{
						m_client.SendChatMsg(m_chatString, a_local: false);
					}
					else
					{
	...
	public void AddString(string a_str)
	{
		...
		if (!m_qmunicator.IsActive(a_ignoreAnimation: false) || !m_isActive)
		{
			m_unreadMsgIndicator.SetActive(value: true);
			m_unreadMsgIndicator2.SetActive(value: true);

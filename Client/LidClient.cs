...
public class LidClient : LidgrenPeer
{
	...
	private void InitStaticBuildings()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if (0 < array[i].m_type)
			{
				...
				remoteBuilding.Init(array[i].transform.position, array[i].m_type, a_isMine: false, a_isStatic: true);
			}
	...
	private void InitCars()
	{
		...
		ref Vector3 reference = ref m_carSeatOffsets[0];
		reference = new Vector3(-0.5f, 0.8f, 0.37f);
		ref Vector3 reference2 = ref m_carSeatOffsets[1];
		reference2 = new Vector3(0.5f, 0.8f, 0.37f);
		ref Vector3 reference3 = ref m_carSeatOffsets[2];
		reference3 = new Vector3(-0.45f, 1.15f, -0.82f);
		ref Vector3 reference4 = ref m_carSeatOffsets[3];
		reference4 = new Vector3(0.45f, 1.15f, -0.82f);
	}
	...
	private void onUpdatePlayers(NetIncomingMessage a_msg)
	{
		if (-1 < m_myOnlineId)
		{
			...
			while (a_msg.PositionInBytes < a_msg.LengthBytes && GetAndUpdatePlayerOrNpc(a_msg, a_isPlayer: true))
			{
			}
	...
	private void onUpdateNpcsItemsBuildings(NetIncomingMessage a_msg)
	{
		...
		GetAndUpdateOwnPlayer(a_msg);
		GetAndUpdateCars(a_msg);
		while (a_msg.PositionInBytes < a_msg.LengthBytes && GetAndUpdatePlayerOrNpc(a_msg, a_isPlayer: false))
		{
		}
	...
	{
		ePartyFeedback ePartyFeedback2 = (ePartyFeedback)a_msg.ReadByte();
		string text = a_msg.ReadString();
		switch (ePartyFeedback2)
		{
		case ePartyFeedback.invite:
			m_popupIdInvite = m_popupGui.ShowGui(a_show: true, text + LNG.Get("PARTY_POPUP_INVITED"));
			break;
		case ePartyFeedback.errorAlreadyInParty:
			m_popupGui.ShowGui(a_show: true, text + LNG.Get("PARTY_POPUP_ALREADY_IN_PARTY"));
			break;
		case ePartyFeedback.kicked:
			m_popupGui.ShowGui(a_show: true, text + LNG.Get("PARTY_POPUP_KICKED"));
			break;
		case ePartyFeedback.prodemoted:
			m_popupGui.ShowGui(a_show: true, text + LNG.Get("PARTY_POPUP_PRODEMOTED"));
			break;
		case ePartyFeedback.partyFull:
			m_popupGui.ShowGui(a_show: true, LNG.Get("PARTY_POPUP_FULL"));
			break;
		}
		...
		for (int i = 0; i < m_missionObjs.Length; i++)
		{
			...
			for (int j = 0; j < m_missions.Count; j++)
			{
				if (m_missionObjs[i].IsMission(m_missions[j]))
				{
					...
					m_missionObjs[i].SetOnOff(a_on: true);
					flag = true;
					break;
				}
			}
			if (!flag)
			{
				m_missionObjs[i].SetOnOff(a_on: false);
			}
		}
	}
	...
	private void onRemoveClient(NetIncomingMessage msg)
	{
		...
		ref CharData reference = ref m_playerData[num];
		reference = new CharData(eCharType.ePlayer);
	...
	private void GetAndUpdateOwnPlayer(NetIncomingMessage a_msg)
	{
		...
		float a_health = (int)(b = (byte)(b & 0x7Fu));
		byte b2 = a_msg.ReadByte();
		m_isInVehicle = 128 == (b2 & 0x80);
		float a_energy = (int)(b2 = (byte)(b2 & 0x7Fu));
		if (!m_isInVehicle)
		{
	...
	private void UpdateOrSpawnBuilding(int a_type, Vector3 a_pos, float a_rot, float a_health, bool a_isMine)
	{
		if (0f < a_health)
		{
			...
			if (null == remoteBuilding)
			{
				...
				remoteBuilding.Init(a_pos, a_type, a_isMine, a_isStatic: false);

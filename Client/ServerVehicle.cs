...
public class ServerVehicle : MonoBehaviour
{
	...
	private void KillAndResetPassengers()
	{
		for (int i = 0; i < 4; i++)
		{
			if (m_data.passengerIds[i] != -1)
			{
				if (null != m_server)
				{
					m_server.GetPlayerByOnlineid(m_data.passengerIds[i])?.ChangeHealthBy(-10000f);
				}
				if (i == 0)
				{
					m_vehicle.AssignInput(0f, 0f, a_space: false);
				}
	...
	private void CausedDamage(float a_dmg)
	{
		...
		{
			m_server.GetPlayerByOnlineid(m_data.passengerIds[0])?.ChangeKarmaBy(Mathf.Clamp(a_dmg, 0f, 100f) * -0.5f);
		}
	}
	...
	public void DestroyCarAndForceExitPassengers()
	{
		for (int i = 0; i < 4; i++)
		{
			if (m_data.passengerIds[i] != -1)
			{
				if (null != m_server)
				{
					m_server.GetPlayerByOnlineid(m_data.passengerIds[i])?.ExitVehicle(a_force: true);
				}
				m_data.passengerIds[i] = -1;
			}
		}
	...
	public Vector3 GetPassengerExitPos(int a_id)
	{
		...
		position.y = 0f;
		Vector3 vector = new Vector3(m_vehicle.transform.right.x, 0f, m_vehicle.transform.right.z);
		Vector3 normalized = vector.normalized;
		Vector3 vector2 = new Vector3(m_vehicle.transform.forward.x, 0f, m_vehicle.transform.forward.z);
		...
		for (int i = 0; i < 4; i++)
		{
			if (a_id == m_data.passengerIds[i])
			{
				...
				float num2 = 0f - num;
				float num3 = ((2 <= i) ? (-1.5f) : 0f) + Random.Range(-0.5f, 0.5f);
				float num4 = 0.6f;
				Vector3 vector3 = position + normalized * num + normalized2 * num3;
				Vector3 vector4 = position + normalized * num2 + normalized2 * num3;
				vector3.y = (vector4.y = num4 + 0.2f);
				if (!Raycaster.CheckSphere(vector3, num4) && 0.8f < Util.GetTerrainHeight(vector3))
				{
					result = vector3;
				}
				else if (!Raycaster.CheckSphere(vector4, num4) && 0.8f < Util.GetTerrainHeight(vector4))
				{
					result = vector4;
				}
				break;

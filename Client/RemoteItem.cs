...
public class RemoteItem : MonoBehaviour
{
	...
	public void Init(Vector3 a_pos, int a_type, int a_amount, bool a_isContainerItem)
	{
		...
		m_isInventoryItem = m_isInventoryOrContainerItem && base.transform.position.x < 5f;
		InstantiateItem();
	...
	private void Update()
	{
		...
		if (null != m_riseItemEffect)
		{
			...
			if (m_riseItemEffect.transform.position.y > 6f)
			{
	...
	public void SwitchVisibility()
	{
		...
		if (!m_visible && !m_isInventoryOrContainerItem)
		{
			m_riseItemEffect = InstantiateItem(a_justForEffect: true);
		}
	}
}

...
public class BodyAISimple : BodyBase
{
			...
			if (Vector3.zero != vector)
			{
				...
				if (Time.time > m_nextAttackTime)
				{
					...
					if (a_target == m_enemy && m_causeCondition != eCondition.none)
					{
						...
						((!(null != component)) ? null : component.GetServerPlayer())?.SetCondition(m_causeCondition, a_state: true);
					}
	...
	private void HandleStuckOnPath(float a_deltaTime)
	{
		if (m_agent.enabled && m_agent.hasPath && !IsMoving())
		{
			...
			{
				float num = ((Random.Range(0, 2) != 0) ? (-1f) : 1f);
				Vector3 vector = base.transform.right * Random.Range(4f, 12f) * num + base.transform.forward * Random.Range(-2f, 0f);
				GoTo(base.transform.position + vector);
				m_stuckDuration = 0f;
				m_nextRepositionTime = Time.time + vector.magnitude / m_agent.speed;
			}
		}
	}
	...
	private void DropLoot()
	{
		...
		for (int i = 0; i < m_itemDrops.Count; i++)
		{
			if (m_itemDrops[i] != null && Random.Range(0, 100) < m_itemDrops[i].chance)
			{
				...
				int num3 = 4;
				Vector3 vector = new Vector3((float)Random.Range(-num3, num3) * 0.1f, 0f, (float)Random.Range(-num3, num3) * 0.1f);
				m_server.CreateFreeWorldItem(num, num2, base.transform.position + vector);
			}
		}
	}

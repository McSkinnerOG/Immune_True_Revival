...
public class SteamInventoryGUI : MonoBehaviour
{
	...
	{
		m_contextMenu.SetActive(value: false);
		RequestSteamInventory();
	}
			...
			if (punOutItemsArraySize != 0)
			{
				...
				else
				{
					for (int i = 0; i < array.Length; i++)
					{
						...
						{
							if (array[i].m_itemId.m_SteamItemInstanceID == m_itemDetails[j].m_itemId.m_SteamItemInstanceID && 0 < (array[i].m_unFlags & 0x100))
							{
				...
				{
					if (m_itemDetails[l].m_iDefinition.m_SteamItemDef == lookItemDef)
					{
						flag = false;
					}
					else if (m_itemDetails[l].m_iDefinition.m_SteamItemDef == skinItemDef)
					{
						flag2 = false;
					}
					if (3000 > m_itemDetails[l].m_iDefinition.m_SteamItemDef)
					{
						num2++;
					}
				}
	...
	private void UpdateInventoryDisplay()
	{
		...
		if (m_itemDetails != null && num < m_itemDetails.Count)
		{
			...
			for (int j = 0; j < m_itemDetails.Count; j++)
			{
				...
				{
					int num3 = j - num;
					JSONNode item = JsonItems.GetItem(m_itemDetails[j].m_iDefinition.m_SteamItemDef);
					if (null != item)
					{
						...
						componentInChildren2.renderer.material.mainTexture = Resources.Load<Texture>("inventory_steam/inventory_s_" + m_itemDetails[j].m_iDefinition.m_SteamItemDef);
	...
	private bool EquipOrOpenItem(int a_index)
	{
		...
		{
			int steamItemDef = m_itemDetails[a_index].m_iDefinition.m_SteamItemDef;
			ulong steamItemInstanceID = m_itemDetails[a_index].m_itemId.m_SteamItemInstanceID;
			if (10000 > steamItemDef)
			{
				if (Global.isSteamActive)
				{
					SteamItemDetails_t steamItemDetails_t = ((steamItemDef != 3000) ? GetRandomItemFromInventory(3000, 3000) : GetRandomItemFromInventory(2000, 2999));
					ulong steamItemInstanceID2 = steamItemDetails_t.m_itemId.m_SteamItemInstanceID;
					if (steamItemInstanceID2 != 0L)
					{
						m_generatorDefId = ((steamItemDef != 3000) ? steamItemDef : steamItemDetails_t.m_iDefinition.m_SteamItemDef) - 1000;
	...
	private SteamItemDetails_t GetRandomItemFromInventory(int a_defIdFrom, int a_defIdTo)
	{
		if (m_itemDetails != null)
		{
			...
			{
				if (m_itemDetails[i].m_iDefinition.m_SteamItemDef <= a_defIdTo && m_itemDetails[i].m_iDefinition.m_SteamItemDef >= a_defIdFrom)
				{
	...
	private bool SellItem(int a_index)
	{
		...
		if (m_itemDetails != null && a_index > -1 && a_index < m_itemDetails.Count)
		{
			...
			string text = "http://steamcommunity.com/profiles/" + num + "/inventory#" + 348670 + "_2_" + m_itemDetails[a_index].m_itemId.m_SteamItemInstanceID;
			if (Global.isSteamActive)
			{
	...
	private int GetClickedItemIndex(string a_clickedBtnName)
	{
		...
		if (array != null && 1 < array.Length && "sii" == array[0])
		{
			...
			if (num > -1 && m_itemDetails != null && num < m_itemDetails.Count)
			{
				...
				int steamItemDef = m_itemDetails[num].m_iDefinition.m_SteamItemDef;
				...
				result = num;
				m_contextMenu.SetActive(value: true);
			}
		}
		return result;
	}

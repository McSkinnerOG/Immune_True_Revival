...
public class InventoryGUI : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			m_guiSteamInventory.SetActive(value: false);
			m_guiBig.SetActive(value: false);
			m_guiItemInfo.SetActive(value: false);
		}
			...
			for (int i = 0; i < m_items.Length; i++)
			{
				...
				if (m_items[i].m_isInventoryOrContainerItem)
				{
					...
					else if (!m_items[i].IsVisible())
					{
						...
						{
							float num = ((!(m_items[i].transform.localPosition.x < 6f)) ? GetShopBuyMultiplier() : GetShopSellMultiplier());
	...
	public void ShowInfo(Vector3 a_pos)
	{
		...
		if (num != -1)
		{
			...
			m_guiInfoStats.text = Items.GetStatsText(num, a_amount, a_displayValue: true);
			m_guiItemInfo.SetActive(value: true);
		}
		else
		{
			m_guiItemInfo.SetActive(value: false);
	...
	public bool HasFood()
	{
		if (m_items != null)
		{
			for (int i = 0; i < m_items.Length; i++)
			{
				if (null != m_items[i] && m_items[i].m_isInventoryItem && Items.IsEatable(m_items[i].m_type) && Items.GetItemDef(m_items[i].m_type).healing > 0f)
				{
					return true;
				}
			}
	...
	public bool HasBuilding()
	{
		if (m_items != null)
		{
			for (int i = 0; i < m_items.Length; i++)
			{
				if (null != m_items[i] && m_items[i].m_isInventoryItem && Items.GetItemDef(m_items[i].m_type).buildingIndex > 0)
				{
					return true;
				}
			}
	...
	{
		m_guiBig.SetActive(value: true);
		m_guiSteamInventory.SetActive(value: true);
	}
}
